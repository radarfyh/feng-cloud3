...
/**
 * redis 保存路由信息，优先级比配置文件高；RouteDefinitionRepository 接口实现类；
 * GatewayAutoConfiguration类中存在bean routeDefinitionRepository，返回return new RedisRouteDefinitionWriter(redisTemplate)，从而把RedisRouteDefinitionWriter注入到路由加载链中
 * 下次请求到达时，触发 getRoutes() 方法，通过this.cachedRoutes = this.delegate.getRoutes().collectList().block()中delegate调用本组件。
 * 分层缓存：内存缓存 (RouteCacheHolder) → Redis缓存 → 数据库。通过 RefreshRoutesEvent 实现多级缓存一致性
 * 调用链：CachingRouteLocator->>CompositeRouteLocator: getRoutes()->>RedisRouteDefinitionWriter: getRouteDefinitions()-->>CompositeRouteLocator: Flux<RouteDefinition>-->>CachingRouteLocator: Flux<Route>
 */
@Slf4j
@Component
