 ...
 * <p>
 * 使用该注解后，框架会自动启用以下功能：
 * 1. 启用 Spring Security 的资源服务器功能。
 * 2. 启用全局方法安全控制，支持使用 @PreAuthorize、@Secured 等注解进行方法权限控制。
...
@EnableResourceServer  // 启用 OAuth2 资源服务器功能
@EnableGlobalMethodSecurity(prePostEnabled = true)  // 启用方法级安全控制，支持 @PreAuthorize 和 @Secured
@Import({ FengResourceServerAutoConfiguration.class, FengSecurityBeanDefinitionRegistrar.class }) // 引入 Feng 的自定义配置类和 Bean 定义
