...
public class UmpMsgStatisticsServiceImpl extends ServiceImpl<UmpMsgStatisticsMapper, UmpMsgStatistics> implements UmpMsgStatisticsService {
    ...
    public boolean upsertStatistics(LocalDate statDate, String appKey, String msgType,
        ...
        try {
            // 检查是否已存在统计记录
            boolean exists = umpMsgStatisticsMapper.existsStatistics(statDate, appKey, msgType);
            
            if (exists) {
                // 更新现有记录
                int updated = umpMsgStatisticsMapper.updateStatistics(
                ...
                if (updated > 0) {
                    log.debug("更新统计记录成功，日期: {}, 应用: {}, 消息类型: {}", 
                            statDate, appKey, msgType);
                    return true;
                }
            } else {
                // 创建新记录
                UmpMsgStatistics statistics = new UmpMsgStatistics();
                statistics.setStatDate(statDate);
                statistics.setAppKey(appKey);
                statistics.setMsgType(msgType);
                statistics.setSendCount(sendCount != null ? sendCount : 0);
                statistics.setSendSuccessCount(sendSuccessCount != null ? sendSuccessCount : 0);
                statistics.setSendFailedCount(sendFailedCount != null ? sendFailedCount : 0);
                statistics.setReceiveCount(receiveCount != null ? receiveCount : 0);
                statistics.setReadCount(readCount != null ? readCount : 0);
                statistics.setErrorCount(errorCount != null ? errorCount : 0);
                statistics.setRetryCount(retryCount != null ? retryCount : 0);
                
                // 计算平均时间（暂时只设置初始值，后续更新时会重新计算）
                statistics.setAvgProcessTime(processTime != null ? processTime : 0);
                statistics.setAvgReceiveTime(receiveTime != null ? receiveTime : 0);
                statistics.setAvgReadTime(readTime != null ? readTime : 0);
                
                statistics.setCreateTime(LocalDateTime.now());
                statistics.setUpdateTime(LocalDateTime.now());
                
                boolean success = save(statistics);
                if (success) {
                    log.info("创建统计记录成功，日期: {}, 应用: {}, 消息类型: {}", 
                            statDate, appKey, msgType);
                    return true;
                }
            }
            ...
            log.error("更新统计记录失败，日期: {}, 应用: {}, 消息类型: {}", 
                    statDate, appKey, msgType, e);
    ...
    public PerformanceStatisticsVO getPerformanceStatistics(LocalDate startDate, LocalDate endDate, String appKey) {
        ...
        if (perfStats != null) {
            ...
            statisticsVO.setP95ProcessTime(((Number) perfStats.getOrDefault("p95_process_time", 0)).intValue());
            statisticsVO.setP95ReceiveTime(((Number) perfStats.getOrDefault("p95_receive_time", 0)).intValue());
            statisticsVO.setP95ReadTime(((Number) perfStats.getOrDefault("p95_read_time", 0)).intValue());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean generateDailyReport(LocalDate statDate) {
        if (statDate == null) {
            throw new IllegalArgumentException("统计日期不能为空");
        }

        // 在实际应用中，这里会从消息主表和收件箱表等源表中聚合数据
        // 由于是示例，我们这里只记录日志
        log.info("生成每日统计报表，日期: {}", statDate);
        
        // TODO: 实现具体的统计报表生成逻辑
        // 1. 从ump_msg_main表中统计当天的消息数据
        // 2. 从ump_msg_inbox表中统计接收和阅读数据
        // 3. 从ump_system_log表中统计错误和重试数据
        // 4. 将统计结果保存到ump_msg_statistics表
