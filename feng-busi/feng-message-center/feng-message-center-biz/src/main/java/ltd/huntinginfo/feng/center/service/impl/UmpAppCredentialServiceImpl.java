...
public class UmpAppCredentialServiceImpl extends ServiceImpl<UmpAppCredentialMapper, UmpAppCredential> implements UmpAppCredentialService {
    ...
    private final UmpAppCredentialMapper umpAppCredentialMapper;
    ...
    public String createApp(String appKey, String appSecret, String appName, String appType,
        ...
        // 检查应用标识是否已存在
        boolean exists = umpAppCredentialMapper.existsByAppKey(appKey);
        if (exists) {
    ...
    public boolean resetAppSecret(String appId, String newSecret, Integer expireDays) {
        ...
        int updated = umpAppCredentialMapper.updateAppSecret(appId, newSecret, expireTime);
        if (updated > 0) {
    ...
    public AppDetailVO getAppByKey(String appKey) {
        ...
        UmpAppCredential app = umpAppCredentialMapper.selectByAppKey(appKey);
        if (app == null) {
    ...
    public Page<AppPageVO> queryAppPage(AppQueryDTO queryDTO) {
        ...
        queryWrapper.eq(UmpAppCredential::getDelFlag, 0); // 只查询未删除的记录
    ...
    public List<AppDetailVO> getAvailableApps() {
        List<UmpAppCredential> apps = umpAppCredentialMapper.selectAvailableApps();
        return apps.stream()
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int batchEnableApps(List<String> appIds) {
        if (CollectionUtils.isEmpty(appIds)) {
            return 0;
        }

        int updatedCount = umpAppCredentialMapper.batchUpdateStatus(appIds, 1);
        if (updatedCount > 0) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int batchDisableApps(List<String> appIds) {
        if (CollectionUtils.isEmpty(appIds)) {
            return 0;
        }

        int updatedCount = umpAppCredentialMapper.batchUpdateStatus(appIds, 0);
        if (updatedCount > 0) {
    ...
    public boolean isAppAvailable(String appKey) {
        ...
        UmpAppCredential app = umpAppCredentialMapper.selectByAppKey(appKey);
        return app != null && app.getStatus() == 1 && app.getDelFlag() == 0;
    }
    ...
    public boolean validateAppCredential(String appKey, String appSecret) {
        ...
        // 检查应用状态
        if (app.getStatus() != 1 || app.getDelFlag() != 0) {
            log.warn("应用不可用，应用标识: {}, 状态: {}, 删除标记: {}", 
                    appKey, app.getStatus(), app.getDelFlag());
            return false;
        }
    ...
    public AppTotalStatisticsVO getAppStatistics() {
        Map<String, Object> statsMap = umpAppCredentialMapper.selectAppStatistics();
    ...
    public boolean deleteApp(String appId) {
        ...
        boolean success = updateById(app);
        if (success) {
            log.info("应用删除成功，应用ID: {}, 应用标识: {}", appId, app.getAppKey());
        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int batchDeleteApps(List<String> appIds) {
        if (CollectionUtils.isEmpty(appIds)) {
            return 0;
        }

        List<UmpAppCredential> apps = listByIds(appIds);
        if (CollectionUtils.isEmpty(apps)) {
            return 0;
        }

        LocalDateTime now = LocalDateTime.now();
        for (UmpAppCredential app : apps) {
            app.setDelFlag(1);
            app.setUpdateTime(now);
        }

        boolean success = updateBatchById(apps);
        if (success) {
            log.info("批量删除应用成功，数量: {}", apps.size());
            return apps.size();
        }
    ...
    public Map<String, Long> getAppTypeStatistics() {
        List<UmpAppCredential> allApps = list();
        
        Map<String, Long> typeStats = allApps.stream()
                .filter(app -> app.getDelFlag() == 0)
                .collect(Collectors.groupingBy(
                        UmpAppCredential::getAppType,
                        Collectors.counting()
                ));
