...
public class UmpTopicSubscriptionServiceImpl extends ServiceImpl<UmpTopicSubscriptionMapper, UmpTopicSubscription> implements UmpTopicSubscriptionService {
    ...
    public String createSubscription(String topicCode, String appKey,
        ...
        subscription.setSubscriptionConfig(subscriptionConfig);
    ...
    public boolean subscribeTopic(String topicCode, String appKey,
        ...
        if (existing != null) {
            ...
            if (existing.getStatus() == 0) {
                ...
                if (subscriptionConfig != null) {
                    existing.setSubscriptionConfig(subscriptionConfig);
                }
                if (StringUtils.hasText(callbackUrl)) {
                ...
                // 已经是活跃订阅，更新配置
                return updateSubscription(existing.getId(), subscriptionConfig, callbackUrl, pushMode);
            }
        }

        // 创建新订阅
        try {
            createSubscription(topicCode, appKey, subscriptionConfig, callbackUrl, pushMode);
            return true;
        } catch (Exception e) {
            log.error("订阅主题失败", e);
            return false;
    ...
    public Integer updateSubscriptionStats(String topicCode, String appKey,
        ...
        return subscription.getMessageCount();
    }
    ...
    public boolean deleteSubscription(String subscriptionId) {
        ...
        // 逻辑删除
        subscription.setDelFlag(1);
        
        boolean success = updateById(subscription);
        if (success) {
            log.info("订阅删除成功，订阅ID: {}, 主题代码: {}, 应用标识: {}", 
                    subscriptionId, subscription.getTopicCode(), subscription.getAppKey());
        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int batchDeleteSubscriptions(List<String> subscriptionIds) {
        if (CollectionUtils.isEmpty(subscriptionIds)) {
            return 0;
        }

        List<UmpTopicSubscription> subscriptions = listByIds(subscriptionIds);
        if (CollectionUtils.isEmpty(subscriptions)) {
            return 0;
        }

        for (UmpTopicSubscription subscription : subscriptions) {
            subscription.setDelFlag(1);
        }

        boolean success = updateBatchById(subscriptions);
        if (success) {
            log.info("批量删除订阅成功，数量: {}", subscriptions.size());
