...
public class UmpStatusCodeServiceImpl extends ServiceImpl<UmpStatusCodeMapper, UmpStatusCode> implements UmpStatusCodeService {
    ...
    public String createStatusCode(String statusCode, String statusName, String statusDesc,
        ...
        if (StringUtils.hasText(parentCode)) {
            ...
            if (parent == null) {
                ...
                throw new RuntimeException("父状态码不存在");
            }
        }
        ...
        statusCodeEntity.setCreateTime(LocalDateTime.now());
        ...
        if (save(statusCodeEntity)) {
            ...
            throw new RuntimeException("状态码创建失败");
        }
    }
    ...
    public boolean updateStatusCode(String id, String statusName, String statusDesc,
        ...
        if (updated) {
            statusCodeEntity.setUpdateTime(LocalDateTime.now());
    ...
    public StatusCodeStatisticsVO getStatusCodeStatistics() {
        ...
        if (statsMap != null) {
            ...
            if (statsMap.containsKey("category_stats")) {
                ...
                Map<String, Object> categoryMap = (Map<String, Object>) statsMap.get("category_stats");
                categoryMap.forEach((key, value) -> categoryStats.put(key, ((Number) value).longValue()));
            }
            statisticsVO.setCategoryStats(categoryStats);
            
            // 最终状态统计
            statisticsVO.setFinalCount(((Number) statsMap.getOrDefault("final_count", 0)).longValue());
            statisticsVO.setNonFinalCount(((Number) statsMap.getOrDefault("non_final_count", 0)).longValue());
            
            // 可重试统计
            statisticsVO.setRetryableCount(((Number) statsMap.getOrDefault("retryable_count", 0)).longValue());
            statisticsVO.setNonRetryableCount(((Number) statsMap.getOrDefault("non_retryable_count", 0)).longValue());
