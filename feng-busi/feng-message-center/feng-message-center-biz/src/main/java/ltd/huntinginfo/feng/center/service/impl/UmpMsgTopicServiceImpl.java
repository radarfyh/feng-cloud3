...
public class UmpMsgTopicServiceImpl extends ServiceImpl<UmpMsgTopicMapper, UmpMsgTopic> implements UmpMsgTopicService {
    ...
    public boolean isTopicAvailable(String topicCode) {
        ...
        UmpMsgTopic topic = umpMsgTopicMapper.selectByTopicCode(topicCode);
        return topic != null && topic.getStatus() == 1 && topic.getDelFlag() == 0;
    }
    ...
    public boolean deleteTopic(String topicId) {
        ...
        boolean success = updateById(topic);
        if (success) {
            log.info("主题删除成功，主题ID: {}, 主题代码: {}", topicId, topic.getTopicCode());
        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int batchDeleteTopics(List<String> topicIds) {
        if (CollectionUtils.isEmpty(topicIds)) {
            return 0;
        }

        List<UmpMsgTopic> topics = listByIds(topicIds);
        if (CollectionUtils.isEmpty(topics)) {
            return 0;
        }

        LocalDateTime now = LocalDateTime.now();
        for (UmpMsgTopic topic : topics) {
            topic.setDelFlag(1);
            topic.setUpdateTime(now);
        }

        boolean success = updateBatchById(topics);
        if (success) {
            log.info("批量删除主题成功，数量: {}", topics.size());
            return topics.size();
        }
