...
public class AdministrativeDivisionController {
    ...
    @Inner
    @GetMapping("/{areaCode}")
    public R<Map<String, Object>> getAreaByCode(@PathVariable String areaCode) {
    	DictAdministrativeDivision area = administrativeDivisionService.getByCode(areaCode);
        if (area == null) {
            return R.failed("区域代码不存在");
        }
        return R.ok(BeanUtil.beanToMap(area));
    }
    ...
    public R<List<Map<String, Object>>> listAreasByCodes(@RequestBody List<String> areaCodes) {
    	return R.ok(administrativeDivisionService.listAreasByCodes(areaCodes));
    }
    ...
    @Inner
    @PostMapping("/users")
    public R<List<Map<String, Object>>> listUsersByArea(@RequestBody Map<String, Object> query) {
        return R.ok(administrativeDivisionService.listUsersByArea(query));
    }
