...
/**
 * 统一机构（UniqueOrg）管理控制器
 */
@RestController
@AllArgsConstructor
@RequestMapping("/dict/unique-org")
...
public class UniqueOrgController {
    ...
    public R<Map<String, Object>> getByIdForFeign(@PathVariable String id) {
    	UniqueOrgInfoVO org = uniqueOrgService.getById(id);
        if (org == null) {
            return R.failed("ID不存在");
        }
        return R.ok(BeanUtil.beanToMap(org));
    }
    ...
    public R<List<Map<String, Object>>> listByIdsForFeign(
    ...
    public R<List<Map<String, Object>>> listUsersByOrgForFeign(
            @RequestBody Map<String, Object> query) {

        // 1. 从请求参数获取 orgId
        Object orgIdObj = query.get("orgId");
        if (orgIdObj == null) {
            return R.failed("orgId 不能为空");
        }
        ...
        List<UniqueUser> users = uniqueUserService.list(
                Wrappers.<UniqueUser>lambdaQuery()
                        .eq(UniqueUser::getOrgCode, org.getOrgCode())
        );
