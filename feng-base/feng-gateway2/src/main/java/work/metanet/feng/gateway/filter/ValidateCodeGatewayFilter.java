...
import work.metanet.feng.common.core.constant.enums.CaptchaFlagTypeEnum;
import work.metanet.feng.common.core.exception.ValidateCodeException;
...
public class ValidateCodeGatewayFilter extends AbstractGatewayFilterFactory {
    ...
    private boolean isCheckCaptchaClient(ServerHttpRequest request) {
        ...
        // 获取租户拼接区分租户的key
//        String strTenant = request.getHeaders().getFirst(CommonConstants.TENANT_HEADER_KEY);
//        if (StrUtil.isNotBlank(strTenant)) {
//        	String des = JasyptUtil.decrypt(strTenant, JasyptUtil.PBE_ALGORITHMS_MD5_DES, JasyptUtil.PBE_GENERAL_PASSWORD);  
//        	TenantOfHeader objTenant = JSONUtil.toBean(des, TenantOfHeader.class);
//        }
    ...
    private void checkCode(ServerHttpRequest request) {
        String code = request.getQueryParams().getFirst("clickCode");
        ...
        if (CommonConstants.IMAGE_CODE_TYPE.equalsIgnoreCase(randomStr)) {
            ...
            return;
        } else if (CommonConstants.WORD_CODE_TYPE.equalsIgnoreCase(randomStr)) {
            //文字点选
            CaptchaVO vo = new CaptchaVO();
            vo.setCaptchaVerification(code);
            vo.setCaptchaType(CommonConstants.WORD_CODE_TYPE);
            if (!captchaService.verification(vo).isSuccess()) {
                throw new ValidateCodeException("验证码不能为空");
            }
            return;
            ...
            //验证码校验 https://gitee.com/log4j/pig/issues/IWA0D
